  ...
  for $search.sort( *.<p>[0].Str ) {
    ...
    my $rw = (
                        ...
                        .split(/ <.ws> '|' <.ws> /)
      {
        ...
        #say "RW-R: $r ({ $r.ends-with('readwrite') })";

        @perms.push: 'read'           if $r.ends-with('_read'  | '_readable');
        @perms.push: 'write'          if $r.ends-with('_write' | '_writable');
        @perms.append: <read write>   if $r.ends-with('readwrite');
